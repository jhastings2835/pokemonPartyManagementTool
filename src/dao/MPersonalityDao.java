	...
	public List<MPersonalityEntity> selectMPersonality() {
		try {
			...
			while (rs.next()) {
				...
				// 攻撃補正
				mPersonality.setAttackCorrenction(rs.getString("ATTACK_CORRECTION"));
				// 防御補正
				mPersonality.setDefenseCorrenction(rs.getString("DEFENSE_CORRECTION"));
				// 特攻補正
				mPersonality.setSpecialAttackCorrection(rs.getString("SPECIAL_ATTACK_CORRECTION"));
				// 特防補正
				mPersonality.setSpecialDefenseCorrection(rs.getString("SPECIAL_DEFENSE_CORRECTION"));
				// 素早さ補正
				mPersonality.setSpeedCorrenction(rs.getString("SPEED_CORRECTION"));
				// 作成したモデルをリストにつめる
